...
async def add_to_cart(product_id: int, quantity, current_user, database: AsyncSession = Depends(db.get_db)):
    product_info = await database.execute(Product).get(product_id)
    if not product_info:
    ...
    cart_info = await database.execute(Cart).filter(Cart.user_id == current_user.id).first()
    if not cart_info:
...
async def add_items(cart_id: int, product_id: int, quantity: int, database: AsyncSession = Depends(db.get_db)):
    ...
    # product_object = await database.execute(Product).filter(Product.id == product_id)
...
async def remove_cart_item(cart_item_id: int, current_user, database: AsyncSession):
    ...
    cart_item = await database.execute(CartItems).filter(and_(CartItems.id == cart_item_id, CartItems.cart_id == cart.id)).first()
    if not cart_item:
